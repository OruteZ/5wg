...
`Assets/_Project/Scripts/Runtime/`에 적용된다. 여기 적힌 것은 취향이 아니라 **이미 60개 파일이
전부 따르고 있는 것**이므로, 새 파일도 맞춰야 한다. 어기면 그 파일만 튄다.

Unity API 쪽 규약(2D 차원, 입력, 시간, 메모리, 구조)은 `unity-csharp.md`에 있다.
이 문서는 **어떻게 쓰는가**, 그쪽은 **무엇을 쓰는가**다.
...
public 클래스와 자명하지 않은 public 메서드에는 XML `/// <summary>`를 단다.
자기 정당화 주석과 구획 배너는 쓰지 않는다.
