...
`Scripts/Runtime/Enemies/`. 기획 수치는 `enemies.md`에 있고 여기는 구조만 둔다.

스테이지가 적 생산자에게 요구하는 것은 "켜라 / 꺼라 / 다 치워라" 셋뿐이고, 그게
`EnemySpawnerBase`(추상 MonoBehaviour)다. 인터페이스가 아닌 것은 씬이 이 자리를 인스펙터로
배선하기 때문이다.
...
| 에셋 | 담는 것 | 범위 |
| --- | --- | --- |
| `RunPacingPlan` | 시간 곡선 3종, 밀도 상한, 스폰 반경, 엘리트 일정, 마디당 박 수 | 스테이지 간 공유 |
| `StageEnemyPlan` | 회차별 가중치, 지정 웨이브, 보스. 페이싱을 참조한다 | 스테이지마다 하나 |
| `EnemyDefinition` | 적 한 종류의 수치 | 적마다 하나 |

나눈 기준은 **스테이지가 바뀌면 같이 바뀌는가**다. 회차 가중치와 웨이브는 그 스테이지의 적을
이름으로 가리키므로 밖으로 나갈 수 없고, 곡선·상한·엘리트 일정은 판의 구조에서 나온 값이라
스테이지 2가 그대로 재사용한다. 다른 페이스를 원하면 페이싱 에셋을 하나 더 만들어 가리킨다.

**HP와 속도는 배수다.** 시간 곡선은 페이싱에 하나뿐이고 종류별 차이는 거기 곱한다.
보스만 예외로 절대값 모드를 쓴다 — 목표 처치 시간에서 직접 뽑은 25,000을 배수로 옮기면
"133배"라는 아무 말도 아닌 숫자가 된다.

### 시간의 두 단위

**회차·웨이브·엘리트·보스는 마디, 곡선은 초로 잰다.** 앞은 곡 구조상의 위치라 BPM을 따라가야
하고, 뒤는 초당 밸런스라 따라가면 안 된다(DPS·TTK가 BPM 비연동).

디렉터는 경과 시간을 초로 누적하고 클록의 BPM으로 마디를 환산한다. 클록의 박 인덱스를 직접
읽지 않는 것은 `BpmClock.ElapsedSec`가 정지 상태에서 0을 돌려주고, 디버그용 시간 건너뛰기가
클록을 되감을 수 없기 때문이다.

### 결정 사항

- **평상시 스폰과 지정 웨이브는 서로를 멈추지 않는다.** 평상시 천장은 "그 시각의 목표 적 수"와
  평상시 상한 중 낮은 쪽이고, 웨이브는 하드 상한만 지킨다. 보스는 상한을 아예 보지 않는다 —
  막혀서 안 나오면 그 판이 이길 수 없는 판이 된다.
- **너무 멀어진 적은 반납이 아니라 그 자리에서 반대편으로 옮긴다.** 사망 경로를 타지 않으므로
  보상도 나오지 않는다. 보스는 면역이다.
- **박 펄스는 적이 아니라 디렉터가 읽는다.** 클록 진행도를 프레임당 한 번만 읽어 나눠준다.
  한 마리씩 읽으면 수백 마리 × 매 프레임이 된다.
- **엘리트와 보스에 전용 클래스가 없다.** 엘리트는 그 회차 추첨 결과에 배수를 얹은 것이고,
  보스는 넉백·디스폰 면역 두 플래그와 절대 HP뿐이다. 나머지는 전부 정의 값이다.
- **가중치와 등장 시점을 둘 다 본다.** 겹치는 조건이 아니다 — 앰프는 2곡 표에 있지만
  등장 시점이 6분이라 그 사이에는 나오면 안 된다.
