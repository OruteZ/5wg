...
프로파일링으로 잡은 유일한 실제 병목이었다. 적 200마리가 전부 Default 레이어에서 서로 밀며
**접촉점 680개**를 유지하고 있었고, 이게 메인스레드 시간의 절반을 먹었다.
...
- 적끼리 안 밀어도 게임이 성립한다(겹쳐서 몰려오는 건 이 장르에서 오히려 자연스럽다).
- 탄끼리는 둘 다 트리거+리지드바디라 겹칠 때마다 `OnTriggerEnter2D`가 헛돌았다.
- **플레이어 접촉 데미지는 그대로 동작한다.** 플레이어 콜라이더가 트리거고 Default↔Enemy는
  열려 있어서, 겹침 판정 경로는 건드리지 않았다.
- 투사체의 자기 주인 통과는 여전히 코드로 막는다. 레이어 설정에 기대지 않는다는 기존 방침 유지.
