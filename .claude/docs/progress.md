...
## 적 시스템 (브랜치 `feat/enemy`)

노션 `[10-12]`를 구현으로 옮겼다. 작업 씬은 **`Scenes/Stage01_Enemy.unity`**(Stage01 복제본)이고
`Stage01`은 손대지 않았다 — 구 `EnemySpawner`가 그대로 붙어 예전대로 돈다.
구조는 `architecture.md`의 "적과 스폰", 기획 수치는 `enemies.md`에 있다.

- 스테이지↔스포너 계약을 `EnemySpawnerBase`(추상)로 뽑았다. `SceneServices`·`StageDirector`의
  필드 타입도 그쪽으로 바꿨고 씬 참조는 그대로 살아 있다.
- 적 정의 7종 + 보스, 페이싱 1개, 스테이지 편성 1개를 `ScriptableObjects/Enemies/`에 만들었다.
- `PrefabPoolSet<T>`에 `onCreate`와 `CollectActive`를 더했다. 사망 구독과, 매 프레임 도는
  디스폰·박 펄스를 위해서다.

### 타이밍 단위를 마디로 바꾼 이유

처음엔 전부 초로 만들었다가 회차·웨이브·엘리트·보스만 마디로 옮겼다. 곡 길이가 "몇 분"이 아니라
**몇 마디**로 정해지기 때문이다(`BeatTimelineEndSource`가 이미 그 기준이었다).
곡선 3종은 초로 남겼다 — DPS·TTK가 BPM 비연동이라 마디로 옮기면 BPM만 올려도 난이도가 빨라진다.

### 기획이 미정이라 비워 둔 것

| 항목 | 상태 |
| --- | --- |
| 적 7종 중 4종과 보스의 **이름** | `[이름 미정]`. 에셋 파일명은 HP 배수로 구분했다 |
| **기본 적 이동 속도** | 임시 2. 플레이어 이동속도가 정해지면 그 비율로 결정된다 |
| **엘리트 HP·크기·속도 배수** | 전부 1. 그러면 일반 적과 구분되지 않으므로 시작 시 경고를 뱉는다 |
| **보스 처치 보상·전용 스프라이트** | 미정. 지금은 일반 적 프리팹을 크기 3배로 쓴다 |

### 확인한 것 (플레이 모드)

- 5종이 각자 정확한 스탯으로 동시에 나온다(속도 2.8/2.4/2.0/1.8/1.0, 접촉 8/8/10/12/20).
- HP가 시간 곡선을 탄다 — 6분에 앰프가 142.8(= 기본 42 × 배수 3.4)이었다.
- 180마디 웨이브가 앰프 35마리를 한 번에 뿌리고, 앰프의 등장 시점 6분도 지켜진다.
- 플레이어를 60만큼 순간이동시키자 일반 적 148마리가 전부 복귀했고 보스는 187 거리에서 유지됐다.
- 보스: HP 25,000, 넉백·디스폰 면역, 처치 시 `OnBossDefeated` 1회.
- **BPM 60으로 낮추면 같은 마디가 다른 초에 온다** — 180마디 웨이브가 6분이 아니라 12분에 떴다.
  초 기준이었다면 안 움직였을 값이라, 마디 기준이 실제로 먹혔다는 증거다.

**밸런스는 확인하지 않았다.** 6분 지점에서 조작 없이 두면 플레이어가 죽는데, 방치의 결과이지
수치 검증이 아니다.

## 다음 작업

- 상자. 엘리트를 잡으면 `OnEliteDefeated`가 나가지만 받는 쪽이 없다
- 보스 승리 판정. `OnBossDefeated`도 마찬가지고, 4회차 제한 시간도 없다
- 회차 전환(곡 전환·상점 진입). 디렉터는 회차를 가중치 표 고르기에만 쓴다
